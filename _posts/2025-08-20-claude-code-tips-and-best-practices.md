...
**Tip: 使用 `/init` 指令初始化 CLAUDE.md 文件**
> 使用 `/init` 指令來 Initialize a new CLAUDE.md file with codebase documentation，這是與 Claude Code 協作的第一步。

**Tip: 在 CLAUDE.md 記錄專案知識**
> 在專案根目錄的 `CLAUDE.md` 檔案中記錄：
> - 專案架構說明
> - 程式碼風格規範
> - 測試框架與限制
> - 常用指令集
> - 開發環境設定
> - Domain knowhow 與解決方案
...
> 在無關任務之間使用 `/clear`，避免 context window 被填滿。
...
**Tip: 無關任務間使用 `/clear` 分離上下文**
> 完成任務後，若下個任務無關聯，使用 `/clear` 或開新 session。
